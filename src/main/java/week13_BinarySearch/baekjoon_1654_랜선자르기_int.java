...
//int 론 합치면 범위 초과해서  불가
//    1 1
//            2147483647

public class baekjoon_1654_랜선자르기_int {
    ...
    static int count_rope_number(int length) {
    ...
    //2^31 -1들어가면 터진다
    static long ParmatricSearch(int max_rope_length) {
        int min = 0;
        int max = max_rope_length; //
        int save=0;

        while (min < max) {
            int mid = (min + max) / 2;
            int check = count_rope_number(mid);
            //여기서 바로 리턴하면 안되고
            //최대값을 찾고 리턴해줘야 된다
